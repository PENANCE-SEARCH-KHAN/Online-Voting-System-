...
export default function CandidateSummaryPage() {
  ...
  const openModal = (candidate = null, index = null) => {
    setEditIndex(index);
    setFormData(
      candidate || { name: '', email: '', region: '', party: '', photo: '', electionType: '' }
    );
    setShowModal(true);
  };
  ...
  const handleSubmit = (e) => {
    ...
    if (editIndex !== null) {
      updated[editIndex] = {
        ...updated[editIndex],
        ...formData,
        photo: formData.photo || updated[editIndex].photo,
      };
    } else {
      updated.push({ ...formData, votes: 0 });
    }
  ...
  // NEW: Print only the table with title
  const printList = () => {
    const printContents = document.getElementById('printable-table').innerHTML;
    const originalContents = document.body.innerHTML;

    document.body.innerHTML = `
      <h1 style="text-align:center; margin-bottom: 20px;">Registered Candidates</h1>
      ${printContents}
    `;
    ...
    <div className="max-w-7xl mx-auto print:p-0">
      ...
      <header className="mb-4 print:hidden">
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400">
          Monitor total candidates, regions represented, and total votes.
        </p>
      </header>

      {/* Summary Cards */}
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mb-4 print:hidden">
        <SummaryCard icon={<User className="w-7 h-7 text-purple-600" />} label="Total Candidates" count={totalCandidates} bgColor="bg-purple-100" hoverBgColor="hover:bg-purple-200" />
        <SummaryCard icon={<MapPin className="w-7 h-7 text-teal-600" />} label="Regions with Candidates" count={uniqueRegionsCount} bgColor="bg-teal-100" hoverBgColor="hover:bg-teal-200" />
        <SummaryCard icon={<BarChart2 className="w-7 h-7 text-indigo-600" />} label="Total Votes" count={totalVotes} bgColor="bg-indigo-100" hoverBgColor="hover:bg-indigo-200" />
      </div>
      ...
      <div className="flex justify-between items-center mb-3 print:hidden">
        ...
        <div className="flex gap-3">
          <button onClick={() => openModal()} className="bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-2 px-4 rounded-lg shadow transition">
            Register Candidate
          </button>
          <button onClick={printList} className="flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white font-medium py-2 px-4 rounded-lg shadow transition">
            <Printer className="w-5 h-5" /> Print List
          </button>
        </div>
      </div>

      {/* Table - add id for print */}
      <div id="printable-table" className="overflow-x-auto rounded-lg shadow border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900">
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          <thead className="bg-indigo-100 dark:bg-indigo-900">
            <tr>
              {['Profile', 'Full Name', 'Email', 'Party', 'Region', 'Election Type', 'Votes', 'Actions'].map((head, i) => (
                <th key={i} className="px-6 py-3 text-left text-xs font-bold uppercase tracking-wider text-gray-700 dark:text-gray-200">{head}</th>
              ))}
            </tr>
          </thead>
            ...
            {candidates.map((c, idx) => (
              <tr key={idx} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                ...
                <td className="px-6 py-4 flex gap-2">
                  <button onClick={() => openModal(c, idx)} className="text-indigo-600 hover:text-indigo-800">
                    <Edit3 className="w-5 h-5" />
                  </button>
                  <button onClick={() => setConfirmDeleteIndex(idx)} className="text-red-600 hover:text-red-800">
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg w-full max-w-md relative">
            <button onClick={closeModal} className="absolute top-3 right-3 text-gray-500 hover:text-red-500">
              <X className="w-5 h-5" />
            </button>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <select value={formData.region} onChange={(e) => setFormData({ ...formData, region: e.target.value })} required className="w-full px-4 py-2 border rounded-lg dark:bg-gray-700 dark:text-white">
                <option value="">Select Region</option>
                {cameroonRegions.map((region, idx) => (
                  <option key={idx} value={region}>{region}</option>
                ))}
              </select>
              <select value={formData.electionType} onChange={(e) => setFormData({ ...formData, electionType: e.target.value })} required className="w-full px-4 py-2 border rounded-lg dark:bg-gray-700 dark:text-white">
              ...
              {formData.photo && <img src={formData.photo} alt="Preview" className="w-10 h-10 rounded-full object-cover" />}
      ...
      {confirmDeleteIndex !== null && (
        <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg w-full max-w-sm relative">
            <button onClick={() => setConfirmDeleteIndex(null)} className="absolute top-3 right-3 text-gray-500 hover:text-red-500">
              <X className="w-5 h-5" />
            </button>
            <h2 className="text-lg font-semibold mb-4 text-gray-800 dark:text-white">Confirm Deletion</h2>
            <p className="text-gray-600 dark:text-gray-300 mb-6">
              Are you sure you want to delete candidate{' '}
              <strong>{candidates[confirmDeleteIndex]?.name}</strong>?
            </p>
            <div className="flex justify-end gap-3">
